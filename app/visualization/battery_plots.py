...
def plot_voltage_vs_time(df: pd.DataFrame,
    ...
    if step_number_col in processed_df.columns and step_type_col in processed_df.columns:
        # Identify step transitions (where step_number changes)
        # We compare with previous row's step_number
        is_transition = processed_df[step_number_col] != processed_df[step_number_col].shift(1)
        steps = processed_df[is_transition]

        # Limit number of annotations to prevent performance issues
        MAX_ANNOTATIONS = 50
        if len(steps) > MAX_ANNOTATIONS:
             steps = steps.head(0) # Skip iterating if too many
