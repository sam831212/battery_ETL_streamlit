...
def plot_capacity_vs_voltage(df: pd.DataFrame,
    ...
    if voltage_col not in df.columns or capacity_col not in df.columns:
        fig = go.Figure()
        fig.update_layout(
            title=f"Cannot create plot: Missing required columns ({voltage_col}, {capacity_col})"
        )
        return fig
    ...
    if step_type_col in processed_df.columns and step_number_col in processed_df.columns:
        ...
        filtered_df = processed_df[processed_df[step_type_col].isin(step_types_to_plot)]
        
        if filtered_df.empty:
            # No charge/discharge data, just plot all data
            fig.add_trace(
                go.Scatter(
                    x=processed_df[voltage_col],
            ...
            for step_type in step_types_to_plot:
                step_type_df = filtered_df[filtered_df[step_type_col] == step_type]
                
                if step_type_df.empty:
                    continue
                
                for step_number, step_df in step_type_df.groupby(step_number_col):
                    # Sort by voltage to get clean curves
                    if step_type == 'discharge':
                        step_df = step_df.sort_values(voltage_col, ascending=False)
                    else:
                        step_df = step_df.sort_values(voltage_col, ascending=True)
    ...
    if highlight_anomalies:
        ...
        if voltage_anomaly_col in processed_df.columns and processed_df[voltage_anomaly_col].any():
            fig = add_anomaly_markers(
                fig, processed_df, voltage_col, capacity_col, 
                anomaly_col=voltage_anomaly_col, name='Voltage Anomalies'
            )
        
        # Check for capacity anomalies
        capacity_anomaly_col = f'{capacity_col}_is_anomaly'
        if capacity_anomaly_col in processed_df.columns and processed_df[capacity_anomaly_col].any():
            fig = add_anomaly_markers(
                fig, processed_df, voltage_col, capacity_col, 
                anomaly_col=capacity_anomaly_col, name='Capacity Anomalies'
            )
...
def plot_voltage_vs_time(df: pd.DataFrame,
    ...
    if voltage_col not in df.columns or time_col not in df.columns:
        fig = go.Figure()
        fig.update_layout(
            title=f"Cannot create plot: Missing required columns ({voltage_col}, {time_col})"
        )
        return fig
    ...
    if step_number_col in processed_df.columns and step_type_col in processed_df.columns:
        ...
        steps = processed_df[[time_col, step_number_col, step_type_col]].drop_duplicates()
...
def plot_current_vs_time(df: pd.DataFrame,
    ...
    if current_col not in df.columns or time_col not in df.columns:
        fig = go.Figure()
        fig.update_layout(
            title=f"Cannot create plot: Missing required columns ({current_col}, {time_col})"
        )
        return fig
...
@handle_plotting_error
@cache_plot(ttl=300)
def plot_combined_voltage_current(df: pd.DataFrame,
                                voltage_col: str = 'voltage',
                                current_col: str = 'current',
                                time_col: str = 'timestamp',
                                step_type_col: str = 'step_type',
    ...
    if voltage_col not in df.columns or current_col not in df.columns or time_col not in df.columns:
        fig = go.Figure()
        fig.update_layout(
            title=f"Cannot create plot: Missing required columns ({voltage_col}, {current_col}, {time_col})"
        )
        return fig
