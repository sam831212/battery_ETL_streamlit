...
def display_data_tables(step_df: pd.DataFrame, detail_df: pd.DataFrame):
    ...
    with table_tabs[0]:
        ...
        column_order = [
            ...
            'power',            # 功率(W)
            'temperature',       # Aux T1
            'temperature_start', # 開始溫度
            'temperature_end',   # 結束溫度
            'temperature_max',   # 最高溫度
            'temperature_min',   # 最低溫度
        ...
        column_mapping = {
            ...
            'power': '功率(W)',
            'temperature': 'Aux T1',
            'temperature_start': '開始溫度(°C)',
            'temperature_end': '結束溫度(°C)',
            'temperature_max': '最高溫度(°C)',
            'temperature_min': '最低溫度(°C)',
            'temperature_avg': '平均溫度(°C)',
            'duration': '工步執行時間(秒)'
        }
    ...
    with table_tabs[1]:
        ...
        if len(detail_df) > 1000:
            st.info(f"Showing first 1000 records out of {len(detail_df)} total records for performance.")
            display_df = detail_df.head(1000)
        else:
...
def display_visualizations(step_df: pd.DataFrame, detail_df: pd.DataFrame):
    ...
    with viz_tabs[0]:
        ...
        print("[DEBUG] Generating Voltage vs Time plot...")

        try:
            # Use detail data for time series plots
            # Downsample to ~2000 points for performance
            target_points = 2000
            if len(detail_df) > target_points:
                step_size = len(detail_df) // target_points
                st.info(f"Showing plot with ~{target_points} sample points (1 out of every {step_size}) out of {len(detail_df)} for performance.")
                plot_data = detail_df.iloc[::step_size].copy()
            else:
                plot_data = detail_df
    ...
    with viz_tabs[1]:
        ...
        print("[DEBUG] Generating Current vs Time plot...")
    ...
    with viz_tabs[2]:
        ...
        print("[DEBUG] Generating Combined plot...")
