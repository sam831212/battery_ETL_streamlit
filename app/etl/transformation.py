...
def transform_data(steps_df: pd.DataFrame, details_df: pd.DataFrame, 
    ...
    # 1. Calculate C-rate
    print("[DEBUG] Calculating C-rate...")
    if nominal_capacity > 0:
        ...
        steps['c_rate'] = 0.0
        details['c_rate'] = 0.0
    print("[DEBUG] C-rate calculated.")
    
    # 2. Calculate SOC
    print("[DEBUG] Calculating SOC...")
    try:
        steps, details = calculate_soc(steps, details)
        print("[DEBUG] SOC calculated.")
    except Exception as e:
        print(f"[DEBUG] Error calculating SOC: {e}")
    
    print("[DEBUG] Calculating pre_test_rest_time...")
    steps = calculate_pre_test_rest_time(steps)
    
    # Calculate temperature statistics from details if available
    if 'temperature' in details.columns:
        print("[DEBUG] Calculating step temperature statistics...")
        steps = calculate_step_temperature_stats(steps, details)
        print("[DEBUG] Step temperature statistics calculated.")
        
    print("[DEBUG] transform_data finished.")
...
def calculate_step_temperature_stats(steps_df: pd.DataFrame, details_df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate temperature statistics (start, end, min, max, avg) for each step from detail data.
    
    Args:
        steps_df: DataFrame containing step data
        details_df: DataFrame containing detail data
        
    Returns:
        Updated steps DataFrame with temperature statistics
    """
    steps = steps_df.copy()
    
    # Initialize columns
    new_cols = ['temperature_start', 'temperature_end', 'temperature_min', 'temperature_max', 'temperature_avg']
    for col in new_cols:
        if col not in steps.columns:
            steps[col] = None
            
    # Check if necessary columns exist
    if 'step_number' not in steps.columns or 'step_number' not in details_df.columns or 'temperature' not in details_df.columns:
        return steps
        
    # Group details by step number
    # Use aggregation to get min, max, mean, first, last
    # Note: 'first' and 'last' assume details are sorted by time, which they should be
    temp_stats = details_df.groupby('step_number')['temperature'].agg(['first', 'last', 'min', 'max', 'mean'])
    
    # Map back to steps DataFrame
    for step_idx, row in steps.iterrows():
        step_num = row['step_number']
        if step_num in temp_stats.index:
            stats = temp_stats.loc[step_num]
            steps.at[step_idx, 'temperature_start'] = round(stats['first'], 1)
            steps.at[step_idx, 'temperature_end'] = round(stats['last'], 1)
            steps.at[step_idx, 'temperature_min'] = round(stats['min'], 1)
            steps.at[step_idx, 'temperature_max'] = round(stats['max'], 1)
            steps.at[step_idx, 'temperature_avg'] = round(stats['mean'], 1)
            
            # Also set main 'temperature' column to end temperature if it's missing or effectively the same
            if 'temperature' not in steps.columns or pd.isna(steps.at[step_idx, 'temperature']):
                 steps.at[step_idx, 'temperature'] = round(stats['last'], 1)
